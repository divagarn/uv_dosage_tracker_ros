...
import cv2  # Add this import at the top if using OpenCV
...
class CostmapCoverageExplorer:
    ...
    def get_trail_color_regions(self, target_value=-2):
        if self.dose_map is None:
            return []

        data = np.array(self.dose_map.data).reshape(
            (self.dose_map.info.height, self.dose_map.info.width)
        )
        coords = []
        for y in range(self.dose_map.info.height):
            for x in range(self.dose_map.info.width):
                if data[y][x] == target_value:
                    wx = self.dose_map.info.origin.position.x + x * self.dose_map.info.resolution
                    wy = self.dose_map.info.origin.position.y + y * self.dose_map.info.resolution
                    coords.append((wx, wy))
        return coords

    def wait_until_region_turns_green(self, wx, wy, timeout=30.0):
        start = rospy.Time.now()
        rate = rospy.Rate(2)
        res = self.dose_map.info.resolution
        ox = self.dose_map.info.origin.position.x
        oy = self.dose_map.info.origin.position.y
        x = int((wx - ox) / res)
        y = int((wy - oy) / res)

        while not rospy.is_shutdown():
            if rospy.Time.now() - start > rospy.Duration(timeout):
                rospy.logwarn("Timed out waiting for region to turn green.")
                return False

            with self.lock:
                if self.dose_map:
                    h, w = self.dose_map.info.height, self.dose_map.info.width
                    if 0 <= x < w and 0 <= y < h:
                        val = self.dose_map.data[y * w + x]
                        if val == -150:  # Green
                            return True
            rate.sleep()
        return False
    ...
    def find_unvisited_clusters_and_select_goal(self):
        if self.merged_map is None:
            return None

        visited = np.zeros_like(self.merged_map, dtype=bool)
        h, w = self.merged_map.shape
        res = self.raw_map.info.resolution
        ox, oy = self.raw_map.info.origin.position.x, self.raw_map.info.origin.position.y
        ...
        for y in range(h):
            for x in range(w):
                if self.merged_map[y][x] == 0 and not visited[y][x]:
                    cluster = bfs(x, y)
                    if len(cluster) >= 1:
                        ...
                        # Check obstacle buffer
                        if not self.is_far_from_obstacles(int(avg_x), int(avg_y)):
                            continue
    ...
    def run(self):
        ...
        while not rospy.is_shutdown():
            ...
            # else:
            #     rospy.loginfo("No more goals. Scanning full map for missed clusters...")
            #     missed_goal = self.find_unvisited_clusters_and_select_goal()
            #     if missed_goal:
            #         rospy.loginfo("Missed region goal: %s", missed_goal)
            #         success = self.send_goal(missed_goal[0], missed_goal[1], rx, ry)
            #         if success:
            #             rospy.loginfo("Reached missed region goal.")
            #             self.covered_points.append(missed_goal)
            #             self.mark_coverage(*missed_goal)
            #             self.mark_as_covered_in_map(*missed_goal, radius=0.3)
            #             self.rotate_in_place()
            #             failures = 0
            #         else:
            #             rospy.logwarn("Failed to reach missed region.")
            #             failures += 1
            #     else:
            #         rospy.loginfo("No unvisited clusters found.")
            #         failures += 1
        ...
        # # === Phase 2: Dose map trail goal tracking ===
        # rospy.loginfo("Normal coverage done. Starting dose trail goal loop...")

        # trail_goals = self.get_trail_color_regions(target_value=-2)

        # for goal_x, goal_y in trail_goals:
        #     rospy.loginfo("Dose trail goal: (%.2f, %.2f)", goal_x, goal_y)

        #     success = self.send_goal(goal_x, goal_y, rx, ry)
        #     if success:
        #         rospy.loginfo("Goal reached, waiting for region to turn green...")
        #         if self.wait_until_region_turns_green(goal_x, goal_y):
        #             rospy.loginfo("Region turned green.")
        #         else:
        #             rospy.logwarn("Region did not turn green. Skipping...")
        #     else:
        #         rospy.logwarn("Could not reach dose trail goal. Skipping...")

        # rospy.loginfo("All dose trail goals processed. Exploration fully complete.")
        # self.cmd_vel_pub.publish(Twist())
